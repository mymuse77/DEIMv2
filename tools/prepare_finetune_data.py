...
# 新增图片所在目录
NEW_IMG_DIR   = Path(r"D:\AI\Datasets\syperson1")

# 新增 YOLO 标注所在目录（.txt 文件，与图片同名）
NEW_LABEL_DIR = Path(r"D:\AI\Datasets\syperson1\labels_my-project-name_2026-03-23-02-43-27")
...
OUTPUT_JSON = Path(r"D:\AI\Datasets\20250731-ppe2286y\train\_annotations_merged.coco.json")
...
def convert_and_merge():
    ...
    if BASE_COCO_JSON and BASE_COCO_JSON.exists():
        ...
        print(f"       已有图片: {len(coco['images'])} 张，标注: {len(coco['annotations'])} 条")
    else:
    ...
    for cls_id, cls_name in CLASS_NAMES.items():
        ...
        if coco_cat_id not in existing_cat_ids:
            coco["categories"].append({"id": coco_cat_id, "name": cls_name, "supercategory": cls_name})
    ...
    # 3. 收集新图片
    img_files = [p for p in NEW_IMG_DIR.iterdir()
    ...
    print(f"\n{'='*60}")
    print(f"  转换完成！")
    print(f"  新增图片: {new_img_count} 张，新增标注: {new_ann_count} 条，跳过: {skipped} 张")
    print(f"  合并后总图片: {len(coco['images'])} 张，总标注: {len(coco['annotations'])} 条")
    print(f"  输出 JSON: {OUTPUT_JSON}")
    print(f"{'='*60}")
    print("\n[\u4e0b\u4e00\u6b65] \u4fee\u6539\u6570\u636e\u96c6\u914d\u7f6e\u4e2d\u7684 ann_file \u6307\u5411\u5408\u5e76\u540e\u7684 JSON\uff0c\u7136\u540e\u8fd0\u884c\uff1a")
    print("  python train.py -c configs/deimv2/deimv2_hgnetv2_atto_helmet_cpu_highprec.yml \\")
    print("                  -t D:/AI/Git/DEIMv2/outputs/deimv2_hgnetv2_atto_helmet_cpu_highprec/best_stg2.pth")
